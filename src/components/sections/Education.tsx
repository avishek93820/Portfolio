      ...
      <div className="container-narrow">
        ...
        <div className="relative max-w-3xl mx-auto">
          ...
          {EDUCATION.map((item, i) => (
            <Reveal key={`${item.institution}-${item.period}`} delay={i * 0.15}>
              <div
              ...
              >
                ...
                >
                  <div className="glass rounded-2xl p-6 glow-border hover:border-cyan-500/30 transition-colors">
                    <div
                    ...
                    <p className="text-[var(--color-muted)] text-sm">{item.degree}</p>
                    {item.detail && (
                      <p className="text-cyan-400/90 text-sm mt-2 font-mono">{item.detail}</p>
                    )}
                  </div>
                </motion.div>

                <div className="absolute left-4 md:left-1/2 w-3 h-3 rounded-full bg-cyan-400 border-4 border-[var(--color-background)] md:-translate-x-1/2 top-6 shadow-lg shadow-cyan-500/50" />
              </div>
            </Reveal>
          ))}
        </div>
